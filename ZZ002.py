...
#!/usr/bin/python3
'''ZZ001.py
a fancier version of SZ001.py.
(forked off on May 5, 2020).
Main difference: support for role-spacific visualization.
(Enabled by default)

THE PURPOSE OF THIS PROGRAM (from SZ001.py):
...
def init_session():
    ...
    SESSION = {\
           ...
           'HOST': HOST,
           'PORT': PORT}
    SESSION['USE_ROLE_SPECIFIC_VISUALIZATIONS']=False

def reset_editable_file():
    file1 = open("editablefile.py", "r")
    file2 = open("editablefilerunning.py", "w")
    file2.write(file1.read())
    file1.close()
    file2.close()

##def reset_covid_file():
##    file1 = open("COVID.py", "r")
##    file2 = open("COVIDrunning.py", "w")
##    file2.write(file1.read())
##    file1.close()
##    file2.close()
...
def get_roles_for_user(username):
  ...
  if DEBUG: print("In get_roles_for_user:",username,roles_for_user)
...
@app.route('/livecoding')
def index2():
  return render_template('editorfront.html', async_mode=socketio.async_mode,
                         port=PORT)

@socketio.on('update', namespace='/session')
def update(data):
    print("request received to update \n \n \n.")
    f = open("editablefilerunning.py", "w")
    f.write(data)
    f.close()
    PROBLEM.reloading_editable_file()
    emit("success", data, broadcast=True)

@socketio.on('updatecovid', namespace='/session')
def updatecovid(data):
    print("request received to update \n \n \n.")
    f = open("COVID.py", "w")
    f.write(data)
    f.close()
    emit("successcovid", data, broadcast=True)

@socketio.on('updatefeedback', namespace='/session')
def changefeedback(data):
    f=open("feedback.py", "w")
    f.write(data)
    f.close()
    emit("successfeedback", data)
             
@socketio.on('getinitinfo', namespace='/session')
def initialinfo():
    f = open("editablefilerunning.py", "r")
    data=f.read()
    f.close()
    emit("refreshinginit", data)

@socketio.on('getinitleaderboard', namespace='/session')
def initialinfo():
    f = open("COVID.py", "r")
    data=f.read()
    f.close()
    emit("refreshinginitcovid", data)

@socketio.on('getinitleaderboard', namespace='/session')
def leaderboard():
    f=open("leaderboard.py", "r")
    data=f.read()
    f.close()
    emit("starting_leaderboard", data)
...
def get_image(image_filename):
  global CURR_DIR
  if DEBUG: print("Trying to get, at path "+CURR_DIR+", the image: "+image_filename)
...
def command(data):
    ...
    if(i==10):
        try:
            CURRENT_STATE = PROBLEM.editablefilerunning.function_custom1(CURRENT_STATE,0)
            CURRENT_STATE.whose_turn=PROBLEM.next_player(CURRENT_STATE.whose_turn)
        except:
            pass
    if(i==11):
        try:
            CURRENT_STATE = PROBLEM.editablefilerunning.function_custom2(CURRENT_STATE,0)
            CURRENT_STATE.whose_turn=PROBLEM.next_player(CURRENT_STATE.whose_turn)
        except:
            pass
    if(i==12):
        try:
            CURRENT_STATE = PROBLEM.editablefilerunning.function_custom3(CURRENT_STATE,0)
            CURRENT_STATE.whose_turn=PROBLEM.next_player(CURRENT_STATE.whose_turn)
        except:
            pass
    if(i==13):
        try:
            CURRENT_STATE = PROBLEM.editablefilerunning.function_custom4(CURRENT_STATE,0)
            CURRENT_STATE.whose_turn=PROBLEM.next_player(CURRENT_STATE.whose_turn)
        except:
            pass
    if(i==14):
        try:
            CURRENT_STATE = PROBLEM.editablefilerunning.function_custom5(CURRENT_STATE,0)
            CURRENT_STATE.whose_turn=PROBLEM.next_player(CURRENT_STATE.whose_turn)
        except:
            pass
...
def handle_win():
    ...
    f = open("leaderboard.py", "w")
    f.write(list(SESSION['USERNAMES'].keys())[0]+","+PROBLEM.score_calculator(CURRENT_STATE)+",")
...
def load_the_problem_formulation_and_port():
  ...
  if len(sys.argv)<2:
    print('''
       Usage:
python3 ../ZZ002.py <PROBLEM NAME> [PORT]
       For example:
ZZ002.py Missionaries 5000
    ''')
    exit(1)
